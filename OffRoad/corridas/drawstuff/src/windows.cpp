...
static void errorBox (char *title, char *msg, va_list ap)
{
  char s[1000];
  vsprintf (s,msg,ap);
  MessageBox (0, (LPCWSTR)s, (LPCWSTR)title,MB_OK | MB_APPLMODAL | MB_ICONEXCLAMATION);
}
...
static HWND GetConsoleHwnd()
{
  ...
  // that has this title.
  char title[1024];
  wsprintf ((LPWSTR)title, (LPWSTR)"DrawStuff:%d/%d",GetTickCount(),GetCurrentProcessId());
  SetConsoleTitle ((LPWSTR)title);
  Sleep(40);			// ensure window title has been updated
  return FindWindow (NULL,(LPWSTR)title);
}
...
static void drawStuffStartup()
{
  ...
  SetConsoleTitle ((LPWSTR)"DrawStuff Messages");
#endif
  ...
  wc.lpszClassName = (LPWSTR)"SimAppClass";
			...
			dsFunctions *fn, int initial_pause)
{
  ...
  main_window = CreateWindow ((LPWSTR)"SimAppClass",(LPWSTR)title,style,
